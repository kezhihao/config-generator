...
export class GitignoreGenerator extends BaseGenerator {
  ...
  generate(answers: PromptResult): string {
    ...
    const lines: string[] = ['# Config-Generator Generated .gitignore'];

    // Dependencies
    if (types.includes('nodejs')) {
      lines.push('', '# Dependencies', 'node_modules/', 'npm-debug.log*', 'yarn-debug.log*',
        'yarn-error.log*', 'pnpm-debug.log*', 'package-lock.json', 'yarn.lock', 'pnpm-lock.yaml');
    }

    if (types.includes('python')) {
      lines.push('', '# Python', '__pycache__/', '*.py[cod]', '*$py.class', '*.so',
        'Python/', 'build/', 'develop-eggs/', 'dist/', 'downloads/', 'eggs/', '.eggs/', 'lib/',
        'lib64/', 'parts/', 'sdist/', 'var/', 'wheels/', '*.egg-info/', '.installed.cfg',
        '*.egg', 'MANIFEST', 'pip-log.txt', 'pip-delete-this-directory.txt', '.venv/', 'venv/');
    }

    if (types.includes('go')) {
      lines.push('', '# Go', '*.exe', '*.exe~', '*.dll', '*.so', '*.dylib', '*.test',
        '*.out', 'go.work', 'vendor/');
    }
    ...
    if (types.includes('ide')) {
      lines.push('', '# IDE', '.vscode/', '.idea/', '*.swp', '*.swo', '*~',
        '.DS_Store', '.env', '.env.local', '.env.*.local');
    }
    ...
    // Logs
    lines.push('', '# Logs', 'logs/', '*.log', 'npm-debug.log*', 'yarn-debug.log*', 'yarn-error.log*',
