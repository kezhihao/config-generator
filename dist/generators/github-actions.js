...
export class GitHubActionsGenerator extends BaseGenerator {
    ...
    description = 'GitHub Actions 持续集成配置';
    definePrompts() {
        return [
            {
                type: 'select',
    ...
    generate(answers) {
        ...
        switch (projectType) {
            case 'nodejs':
                workflow.jobs.ci.steps.push({ uses: 'actions/setup-node@v4', with: { 'node-version': '20', cache: 'npm' } }, { name: 'Install dependencies', run: 'npm ci' });
                ...
                break;
            case 'python':
                workflow.jobs.ci.steps.push({ uses: 'actions/setup-python@v5', with: { 'python-version': '3.11' } }, { name: 'Install dependencies', run: 'pip install -r requirements.txt' });
                if (tasks.includes('lint')) {
                ...
                break;
            case 'go':
                workflow.jobs.ci.steps.push({ uses: 'actions/setup-go@v5', with: { 'go-version': '1.21' } }, { name: 'Install dependencies', run: 'go mod download' });
                if (tasks.includes('test')) {
                ...
                break;
            case 'rust':
                workflow.jobs.ci.steps.push({ uses: 'actions-rs/toolchain@v1', with: { toolchain: 'stable', override: true } });
                if (tasks.includes('test')) {
                ...
                break;
            case 'docker':
                workflow.jobs.ci.steps.push({ name: 'Set up Docker Buildx', uses: 'docker/setup-buildx-action@v3' }, { name: 'Build', run: 'docker build -t myapp .' });
                break;
        }
        // 将 YAML 对象转换为字符串
